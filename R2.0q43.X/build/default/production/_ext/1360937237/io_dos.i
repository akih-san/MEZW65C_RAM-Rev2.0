...
static int write_sector(void) {
 ...
 read_sram(req_tbl.data_adr, disk_buf, 128);
...
static int read_sector(void) {
 ...
 if ((fres = f_read(filep, disk_buf, 128, &n)) != FR_OK || n != 128) {
  ...
  write_sram(req_tbl.data_adr, disk_buf, 128);
...
void bus_master_operation(void) {
 ...
 if (req_tbl.UREQ_COM) {
  ...
  write_sram(bioreq_ubuffadr, (uint8_t *)&req_tbl, 2);
 }
 else {
  buf = &tmp_buf[0][0];
  ...
  write_sram(bioreq_cbuffadr, (uint8_t *)&req_tbl.CREQ_COM, 2);
 }
