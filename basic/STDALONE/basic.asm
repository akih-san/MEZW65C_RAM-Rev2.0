...
; MEZW65C_RAM version
;
; This program was ported for the mezzanine board MEZW65C_RAM
; by Akihito Honda. 2024.10
; X(Twitter)  @akih_san
;
; Mezzanine board MEZW65C_RAM works on EMUZ80 SBC
;
; EMUZ80
...
;               equ $70       ; End of descriptor stack
...
;               equ $D7       ; end of get BASIC char subroutine
; end bulk initialize from LAB_2CEE at LAB_2D4E

Rbyte4          equ $D8       ; extra PRNG byte
...
NmiBase         equ $DC       ; NMI handler enabled/setup/triggered flags
                              ; bit function
                              ; === ========
                              ; 7   interrupt enabled
                              ; 6   interrupt setup
                              ; 5   interrupt happened
;               equ $DD       ; NMI handler addr low byte
;               equ $DE       ; NMI handler addr high byte
IrqBase         equ $DF       ; IRQ handler enabled/setup/triggered flags
;               equ $E0       ; IRQ handler addr low byte
;               equ $E1       ; IRQ handler addr high byte
...
TK_RETIRQ       equ TK_GOSUB+1      ; RETIRQ token
TK_RETNMI       equ TK_RETIRQ+1     ; RETNMI token
TK_RETURN       equ TK_RETNMI+1     ; RETURN token
...
TK_IRQ          equ TK_BITCLR+1     ; IRQ token
TK_NMI          equ TK_IRQ+1        ; NMI token

; secondary command tokens, can't start a statement

TK_TAB          equ TK_NMI+1        ; TAB token
...
ccflag          equ $0200     ; BASIC CTRL-C flag, 00 = enabled, 01 = dis
...
;Ibuffs          equ IRQ_vec+$14
Ibuffs          equ VEC_SV+$16
                              ; start of input buffer after IRQ/NMI code
Ibuffe          equ Ibuffs+$47; end of input buffer

Ram_base        equ $0300     ; start of user RAM (set as needed, should be page aligned)
Ram_top          equ $D700     ; end of user RAM+1 (set as needed, should be page aligned)

UREQ_COM      equ       $18     ; CONIN/CONOUT request command (SYS)
CREQ_COM      equ       $1A     ; CONIN/CONOUT request command (User)
CBI_CHR         equ       $1B     ; charcter (CONIN/CONOUT) or number of strings


Stack_floor     equ 16        ; bytes left free on stack for background interrupts
...
start_p	dw	Ram_top		; start address (Low)
	dw	0		; (high)
...
; *** replace
;      CPY   #<Ram_base+$100      ; compare with start of RAM+$100 high byte
; +++ with
      CPY   #>Ram_base+$100      ; compare with start of RAM+$100 high byte
; *** end patch    2.22p5.0 ***
