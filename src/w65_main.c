...
uint8_t cin_file[BUF_SIZE];		//512bytes
		...
		if (fr == FR_OK) {
			if ( !adr ) {
				if ( raw_addr ) {
					...
					if (!load_flg) {
						if ( header->bios_sw == 1 ) {	// file is user program(bios call)
							binv.sw = 1;
...
next_char:
	...
	if ( !fin_cnt ) {
		frd_ptr = 0;
		if ( f_read( &in_fl, &cin_file[0], BUF_SIZE, (UINT *)&fin_cnt ) ) {
			printf("File read error.\r\n");
			f_close( &in_fl );
